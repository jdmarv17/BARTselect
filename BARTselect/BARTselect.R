...
### PROBABLY DEPRECATED
# get.indicators = function(filtered_trees, actual_samps, vars, num_threads_wrangle) {
# 
#   ##### This function will take filtered trees from BART (no terminal nodes) and give indicators:
#   ##### Return 1: a data frame of variable inclusion indicators
#   #####           i.e. one row for each tree with indicator = 1 if var in the tree
#   # ARGS:
#   # `filtered_trees` = data frame with splitting vars for each tree (no terminal nodes)
#   # `actual_samps` = number of samples taking burning into account
#   # `vars` = covariate names
# 
#   # this is a list of variables in each tree in bart run
#   tree.var.list =
#     filtered_trees %>%
#     group_by(., tree, sample) %>%
#     group_split() %>%
#     mclapply(., function(x) x[,"var"], mc.cores = num_threads_wrangle) %>% # at this point there is a list of tibbles, each one has a row for each variable in the tree
#     lapply(., unlist) %>%
#     lapply(., unname) # at this point its a long list of vectors, each one contains indices of variables in tree
# 
#   # get indicator vector from each tree variable vector (should have length = actual_samps * num_trees)
#   indicator.list = mclapply(X = tree.var.list, FUN = make.indicators,
#                             p = length(vars), mc.cores = num_threads_wrangle)
# 
#   # collapse to data frame of indicators to return
#   to.return = as.data.frame(data.table::transpose(indicator.list), col.names = vars)
# 
#   return(to.return)
# }
...
#   data.mat = model.matrix(formula, data = data)
#   response = as.character(formula[2])
# 
#   if ("(Intercept)" %in% colnames(data.mat)) {
#     data_to_use = as.data.frame(data.mat[,-1])
#     vars = colnames(data_to_use)
#     data_to_use[,response] = data[,response]
#   } else {
#     data_to_use = as.data.frame(data.mat)
#     vars = colnames(data_to_use)
#     data_to_use[,response] = data[,response]
#   }
#   
...
#     
#     # create null data frame where y vector is randomized 
#     null_response = sample(data_to_use[,response], size = nrow(data_to_use), replace = FALSE)
#     data_to_use[,response] = null_response
#     
#     # null BART run
#     null_bart_run = run_bart(formula = formula, data = data_to_use, 
#                              num_burn = num_burn, num_samps = num_samps,
#                              num_chains = num_chains, num_trees = num_trees, 
#                              num_thin = num_thin, prior_power = prior_power,
#                              prior_base = prior_base, num_threads_bart = num_threads_bart,
...
#     
#     # get null co-inclusion probabilities using get_posteriors
#     posteriors = get_posteriors(formula, data_to_use, indicators) 
#     
#     conditionals_df = posteriors[[1]]
#     diff_df = posteriors[[2]]
#     marginals = posteriors[[3]]
...
# bart.pipe = function(formula, data, 
#                      num_trees = 10, num_samps = 5000,
#                      num_burn = 5000,  num_thin = 5,
...
#                      num_threads_wrangle = parallel::detectCores(),
#                      set.diff.thresh = FALSE, diff.thresh,
#                      intercept = FALSE, response.type = "continuous") {
#   
...
#   
#   # data and formula info 
#   data_to_use = model.frame(formula, data = data) 
#   vars = attr(attr(data_to_use, "terms"), "term.labels")
#   response = as.character(formula[2])
#   
#   
#   # print progress
...
#   if (method == "global") {
#     mod.global = fit.model.sim(data = data, response.type = response.type, response = response,
#                                main.effects = var.names, interactions = interact.global,
#                                intercept = intercept)
#     mod.local = NULL
#   } else if (method == "local") {
#     mod.global = NULL
#     mod.local = fit.model.sim(data = data, response.type = response.type, response = response,
